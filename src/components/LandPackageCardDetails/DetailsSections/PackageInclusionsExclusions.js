...
const PackageInclusionsExclusions = ({ packages }) => {
  const { Inclusions, Exclusions } = packages;
  console.log('include',packages)
    ...
    <>
      <Grid container spacing={3}>
        {items && items.length > 0 ? (
          ...
          >
            Keine Artikel{label.toLowerCase()}
          </Typography>
        </Grid>
    ...
    <>
      ...
      {tabValue === 0 && (
        <Box sx={{ marginTop: '1rem' }}>
          ...
          {renderItems(Inclusions, showAllIncludes, toggleShowAllIncludes, "Included in Package")}
        </Box>
      )}
      {tabValue === 1 && (
        <Box sx={{ marginTop: '1rem' }}>
{!isMobile&&<Typography            variant="h6"
          ...
          {renderItems(Exclusions, showAllExclusions, toggleShowAllExclusions, "Excluded in Package")}
        </Box>
      )}
    </>
  ) : (
