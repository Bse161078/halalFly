...
const FlightDetailsSection = ({ flightDetails }) => {
  ...
  return (
    <Box sx={{ mt: 4 }}>
      {/* Header Section */}
      <Typography variant="h5" sx={{    fontSize: isMobile ? '0.65rem' : '1rem', // Adjust font size for mobile
 fontWeight: 'bold', color: '#004e8c', textAlign: 'center', mb: isMobile?1:3 }}>
        Your Flight is Included
      </Typography>
      <Typography variant="body2" sx={{fontSize: isMobile ? '0.55rem' : '1rem', 
         color: '#8d8d8d', textAlign: 'center', mb: isMobile?1:4 }}>
        We have selected the best available flight option from {flightDetails?.length} alternatives.
      </Typography>
        ...
        {flightDetails.map((flight, index) => (
          <Grid item xs={12} sm={6} md={4} key={index}>
            ...
            >
              <Box
                component="img"
                src={flight?.image[0]?.url || 'https://via.placeholder.com/300'} // Airline logo or placeholder
                alt={flight?.flights}
                sx={{ width: '100%', height: isMobile?'150px':'250px', objectFit: isMobile?'cover':'contain', borderRadius: '8px' }} // Contain the image
              />
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="lg" fullWidth>
        ...
        >
          <Typography variant="h6">Flight Image</Typography>
